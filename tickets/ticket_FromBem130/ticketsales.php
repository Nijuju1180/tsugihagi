...
// 権限チェック: ログイン済みかつ販売権限
if (!isset($_SESSION['access_id']) || empty($_SESSION['can_ticket_sales'])) {
    http_response_code(403);
    include __DIR__ . '/header.php';
    echo '<div style="max-width:500px;margin:5em auto;background:#fff;padding:2em;border-radius:8px;box-shadow:0 2px 8px #ccc;font-size:1.2em;color:#c00;">チケット販売権限がありません</div>';
    exit;
}

// --- Ajax: チケット情報取得・バリデーションAPI ---
if (isset($_GET['api']) && $_GET['api'] === 'ticketinfo' && isset($_POST['ticketid'])) {
    ...
    try {
        ...
        if (!$ticket) {
            ...
            echo json_encode(['error' => 'このチケットは既に販売済みです。']);
            exit;
        } else {
...
// --- Ajax: 一括販売処理API ---
if (isset($_GET['api']) && $_GET['api'] === 'bulk_sale' && isset($_POST['ticketids'])) {
    if (!isset($_SESSION['access_id']) || empty($_SESSION['can_ticket_sales'])) {
        http_response_code(403);
        header('Content-Type: application/json');
        echo json_encode(['error' => '権限がありません']);
    ...
    try {
        ...
        foreach ($ticketids as $i => $ticketid) {
            ...
            if (!$ticket) {
                ...
                $results[] = ['ticketid' => $ticketid, 'error' => 'このチケットは既に販売済みです。'];
                $has_error = true;
            } else {
                $stmt = $pdo->prepare('UPDATE ticket SET ticketsalesid = ?, ticketsalestime = NOW() WHERE ticketid = ? AND flavor = ? AND hr = ? AND menu = ?');
                $stmt->execute([$access_id, $ticketid, $flavor, $hr, $menu]);
                $results[] = ['ticketid' => $ticketid, 'success' => '販売処理が完了しました。', 'access_id' => $access_id];
            }
        }
        if ($has_error) {
            $pdo->rollBack();
...
<main style="max-width:700px;margin:2em auto;padding:2em;border-radius:10px;box-shadow:0 2px 8px #ccc;">
  ...
  <form id="salesForm" method="post" style="margin-bottom:2em;" onsubmit="return false;">
    <h3>追加済みチケット一覧</h3>
    <table id="ticketListTable" border="1" cellpadding="8" style="width:100%;margin-bottom:1em;text-align:center;">
      <thead>
    ...
    <button id="bulkSaleBtn" type="button" style="width:100%;padding:1em;font-size:1.1em;background:linear-gradient(45deg,#667eea,#764ba2);color:#fff;border:none;border-radius:8px;">確認画面を表示</button>
  </form>

  <div style="font-size:0.9em;color:#666;">※QRコードは <b>year,hr,menu,flavor,ticketid</b> の形式でコンマ区切りです。<br>※販売処理は未販売のチケットのみ可能です。</div>
</main>
<!-- ZXingライブラリ -->
...
const ticketListTable = document.getElementById('ticketListTable').getElementsByTagName('tbody')[0];
...
function updateTicketTable() {
  ...
  // 削除ボタン
  ticketListTable.querySelectorAll('button[data-idx]').forEach(btn => {
    btn.onclick = e => {
...
function playDoubleBeep() { // QRコード重複読み取り
  try {
    const ctx = new (window.AudioContext || window.webkitAudioContext)();
...
function playDoubleBeepError() { // その他のエラー
  try {
    const ctx = new (window.AudioContext || window.webkitAudioContext)();
    const osc1 = ctx.createOscillator();
    const gain1 = ctx.createGain();
...
confirmOkBtn.onclick = async function() {
  ...
  const res = await fetch('?api=bulk_sale', { method: 'POST', body: form });
  ...
  if (ok && ng === 0) {
    // 成功時はモーダル内容を上書き
    confirmListArea.innerHTML = `<div style='color:#1976d2;font-size:1.2em;padding:2em 0;'>${ok}件の販売処理が完了しました。</div>`;
